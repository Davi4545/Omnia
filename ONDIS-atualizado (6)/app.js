...
const defaultState = {
  ...
  badges: { earned: [] },
  goalsByMonth: {},
  vendorGoalsByMonth: {} // FIX: removida a declaração duplicada
};
...
function renderCurrent(){
  ...
  panel.querySelectorAll('button[data-act]').forEach(btn=>{
    btn.addEventListener('click', (e)=>{
      ...
      if(act === 'finalize') finalizeCurrent(cid);
    });
  });
...
function sellerRevenueDay(sid, tk){
  return state.records.filter(r=> r.sellerId===sid && r.dateKey===tk && r.outcome==='sold')
    .reduce((s,r)=>s+Number(r.value||0),0);
}
function sellerRevenueWeek(sid, mk, week){
  return state.records.filter(r=> r.sellerId===sid && r.monthKey===mk && r.outcome==='sold' && weekOfMonth(r.dateKey)===week)
    .reduce((s,r)=>s+Number(r.value||0),0);
}
...
function evaluateSellerBadgesAfterRecord(rec){
  ...
  // P.A, TVA, TM, Conversão (avaliados a cada venda do mês)
...
// ======================
// ARENA DE RESULTADOS (renderSellerView)
// ======================
function renderSellerView(){
  ...
  function calcArenaScore(records){
    ...
    return { value, pieces, sales, score: Math.round(value + (sales*50) + (pieces*25)) };
  }

  function getSeal(score){
    let current = ARENA_SEALS[0];
    for(const s of ARENA_SEALS){ if(score >= s.min) current = s; }
    return current;
  }
  ...
  view.innerHTML = `
    <section class="card arenaCard">
      ...
      <div class="cardBody">
        ...
        <div id="arenaKpis" class="kpis"></div>
  ...
  const rankEl   = document.getElementById('arenaResultsRanking');
  ...
  // Todos inativos — avisa mas ainda mostra
  if(active.length === 0){
    kpisEl.innerHTML = `<div class="hint">Nenhum vendedor está marcado como <b>ativo</b>.<br>Ative o vendedor em <b>+ Vendedor</b>.</div>`;
    metaEl.innerHTML = '<div class="hint">—</div>';
    yearEl.innerHTML = '<div class="hint">—</div>';
    chartEl.innerHTML= '<div class="hint">—</div>';
    xpEl.innerHTML   = '<div class="hint">—</div>';
    badgesEl.innerHTML = '';
    const btn = document.getElementById('btnSellerDownloadBadges');
    if(btn) btn.onclick = ()=> alert('Ative um vendedor para liberar tudo.');
  }

  const listForPick = active.length ? active : any;
  sel.innerHTML = listForPick.map(s=>`<option value="${s.id}">${s.name||'Vendedor'}${s.active===false?' (inativo)':''}</option>`).join('');

  // Restaura último vendedor selecionado
  state.ui = state.ui || {};
  const wanted = state.ui.sellerViewId || '';
  sel.value = (wanted && listForPick.some(s=>s.id===wanted)) ? wanted : (listForPick[0]?.id || '');
  state.ui.sellerViewId = sel.value || '';

  // Intervalo padrão = mês completo
  ...
  function update(){
    ...
    const fromKey = fromEl.value || (mk2+'-01');
    const toKey   = toEl.value   || (mk2+'-'+String(daysInMonth(mk2)).padStart(2,'0'));

    // Registros do mês (para selos mensais)
    ...
    // Registros do intervalo escolhido (para KPIs e gráfico)
    ...
    metaEl.innerHTML = `
      <div class="row" style="justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap">
        ...
        <div class="tag">Venda record (mês): <b>${formatMoney(best)}</b></div>
      </div>
      <div class="divider"></div>
    ...
    // Sparkline diária do mês
    const days  = daysInMonth(mk2);
    const daily = Array.from({length:days}, (_,i)=>{
      const dk = mk2 + '-' + String(i+1).padStart(2,'0');
      return state.records.filter(r=> r.sellerId===sid && r.dateKey===dk && r.outcome==='sold')
        .reduce((s,r)=>s+Number(r.value||0),0);
    });
    const maxV = Math.max(1, ...daily);
    const wpx=420, hpx=120;
    const pts = daily.map((v,i)=>[ (i/(days-1||1))*wpx, hpx-(v/maxV)*hpx ]);
    ...
    // Ranking da Arena (todos os vendedores ativos no mês)
    const rankRows = listForPick.map(s=>{
      const recs = state.records.filter(r=> r.sellerId===s.id && r.monthKey===mk2);
      const { value, pieces, sales, score } = calcArenaScore(recs);
      return { id:s.id, name:s.name||'Vendedor', value, pieces, sales, score, seal:getSeal(score) };
    ...
    rankEl.innerHTML = `
      <div class="arenaRankingCard">
        <table class="arenaRankingTable">
          <thead>
            <tr><th>#</th><th>Vendedor</th><th>Faturamento</th><th>Peças</th><th>Pontos</th><th>Selo</th></tr>
          </thead>
          <tbody>
            ${rankRows.map((r,i)=>`
              <tr>
    ...
    const badgeCards = badgeOrder.map(defKey=>{
      ...
      return `
        <div class="badgeCard ${unlocked?'':'badgeLocked'}">
          ...
          <div class="badgeDesc" style="margin-top:8px">
            ${unlocked ? ('<span class="tag">ATIVADO</span> '+sub) : '<span class="tag">BLOQUEADO</span>'}
          </div>
        </div>`;
    });

    badgesEl.innerHTML = badgeCards.some(c=>c) ? badgeCards.join('') : '<div class="hint">Nenhum selo ainda. Finalize vendas e metas para destravar. ⭐</div>';
...
function downloadSellerBadges(){
  ...
  const cards = personal + (earned.map(e=>`
    <div style="border:1px solid #e5e7eb;border-radius:14px;padding:12px;margin:10px 0;display:flex;gap:10px;align-items:center">
  ...
  a.download = `selos_${(seller?.name||'vendedor').toLowerCase().replace(/\s+/g,'_')}_${mk}.html`;
  document.body.appendChild(a);
  a.click();
...
function setTab(tab){
  ...
  for(const vid of viewIds){ const el=document.getElementById(vid); if(el) el.style.display="none"; }
  for(const tid of tabIds){ const el=document.getElementById(tid); if(el) el.classList.remove("active"); }

  const show = (vid, tid)=>{
    const v=document.getElementById(vid); if(v) v.style.display="block";
  ...
  try{
    ...
    else if(tab==="seller")   { show("viewSeller","tabSeller"); try{ renderSellerView(); }catch(err){ console.error("renderSellerView failed",err); } }
...
["pW1","pW2","pW3","pW4"].forEach(id=>{ $(id).addEventListener("input", saveGoalsDraftAndAuto); });
...
$("goalVendorSelect").addEventListener("change", ()=>{ try{ renderVendorGoals(); }catch(e){} });
...
$("btnAutoRedistribute").addEventListener("click", ()=>{ renderGoals(); alert("Redistribuição automática aplicada ✅"); });
$("btnCloseWeek").addEventListener("click", closeCurrentWeek);
$("btnReopenWeeks").addEventListener("click", reopenWeeks);

// Delegação segura para Arena (elementos criados dinamicamente)
document.addEventListener("change", (e)=>{
  const t = e.target;
  if(!(t instanceof HTMLElement)) return;
  if(t.id==="sellerViewSelect"){ try{ state.ui=state.ui||{}; state.ui.sellerViewId=t.value; save(state); }catch(e){} try{ renderSellerView(); }catch(e){} }
  if(t.id==="sellerViewMonth") { try{ renderSellerView(); }catch(e){} }
});

document.addEventListener("click", (e)=>{
  const t = e.target;
  if(!(t instanceof HTMLElement)) return;
  if(t.id==="btnSellerDownloadBadges"){ try{ downloadSellerBadges(); }catch(err){ console.error(err); } }
});
...
$("btnDeactivateSeller").addEventListener("click", ()=>{ renderDeactivateModal(); openModal($("deactBack")); });
...
// Ranking modal
$("btnRanking").addEventListener("click", ()=>{ renderRankingModal(); openModal($("rankingBack")); });
...
// Preferência modal
$("btnPreference").addEventListener("click", ()=>{ renderPreferenceModal(); openModal($("prefBack")); });
...
// Timer (múltiplos atendimentos simultâneos)
setInterval(()=>{
  for(const c of (state.currents||[])){
    const t = document.getElementById("timer_"+c.id);
    if(!t) continue;
    t.textContent = Math.floor((nowTs()-(c.startTs||nowTs()))/1000) + "s";
  }
}, 1000);
...
document.getElementById("btnInstall")?.addEventListener("click", async ()=>{
  ...
  if(isIOS()){ openInstallHelp(); return; }
...
document.getElementById("installBack")?.addEventListener("click", (e)=>{
  if(e.target===document.getElementById("installBack")) closeInstallHelp();
});

try{ showInstallButton(); }catch(e){}
